...
function NavIcon({ label, active }: { label: string; active: boolean }) {
  const gradientId = `nav-icon-glass-stroke-${label === '홈' ? 'home' : label === '리스트' ? 'list' : label === '라운지' ? 'lounge' : 'my'}`
  const glassStroke = (
    <defs>
  ...
  if (label === '리스트') {
    return (
      <svg viewBox="0 0 12 20" fill="currentColor" aria-hidden="true" className="size-full">
        {glassStroke}
        {active ? (
          <path {...strokeProps} d="M8.59 0a2.58 2.58 0 0 1 2.58 2.41l.17 1.84H.66l.17-1.84A2.58 2.58 0 0 1 3.41 0h5.18ZM12 10.2a5.94 5.94 0 0 1-4.89 5.52v2.23h2.3a.98.98 0 1 1 0 1.9H2.88a.98.98 0 1 1 0-1.9h2.3v-2.23A5.98 5.98 0 0 1 0 10l.37-4.03h11.26L12 10.2Z" />
        ) : (
          <path {...strokeProps} d="M9.43 18.33H6.86v-2.57A5.9 5.9 0 0 0 12 9.91l-.72-7.64A2.56 2.56 0 0 0 8.72 0H3.28A2.56 2.56 0 0 0 .72 2.27L0 9.98a5.9 5.9 0 0 0 5.14 5.78v2.57H2.57a.84.84 0 0 0 0 1.67h6.86a.84.84 0 0 0 0-1.67ZM3.28 1.67h5.44c.45 0 .82.32.86.75l.16 1.75H2.26l.17-1.75c.04-.43.4-.75.85-.75ZM1.71 10.06l.4-4.23H9.9l.39 4.15a4.29 4.29 0 0 1-8.58.08Z" />
        )}
      </svg>
    )
  }
  ...
  return (
    <svg viewBox="0 0 15 20" fill="currentColor" aria-hidden="true" className="size-full">
      {glassStroke}
      {active ? (
        <path {...strokeProps} d="M7.5 11.47A7.5 7.5 0 0 1 15 19a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1 7.5 7.5 0 0 1 7.5-7.53ZM7.5 0a5 5 0 1 1 0 10 5 5 0 0 1 0-10Z" />
      ) : (
        <>
