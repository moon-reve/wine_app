...
function FeedComposer({ photo, onBack }: { photo: string; onBack: () => void }) {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-[430px] bg-white pb-6 text-[#121212] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
    ...
    <form className="space-y-6 px-5 pt-[11px]" onSubmit={(e) => e.preventDefault()}>
      <section><h2 className="mb-[10px] text-sm font-bold">사진 첨부</h2><div className="flex gap-3"><img src={feedCamera} alt="첨부된 사진" className="size-[90px] rounded-[10px] object-cover" /><button type="button" className="size-[90px] rounded-[10px] border border-dashed border-[#d9d9d9] bg-[#f2f2f2] text-[26px] text-[#737373]">+</button></div></section>
      <section><h2 className="mb-[10px] text-sm font-bold">사진 첨부</h2><div className="flex gap-3"><img src={photo} alt="첨부된 사진" className="size-[90px] rounded-[10px] object-cover" /><button type="button" className="size-[90px] rounded-[10px] border border-dashed border-[#d9d9d9] bg-[#f7f6f5] text-[26px] text-[#737373]">+</button></div></section>
