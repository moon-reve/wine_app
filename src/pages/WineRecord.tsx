    ...
    <form className="space-y-[17px] px-5 pt-6" onSubmit={handleSubmit}>
      ...
      <section><h2 className="mb-2 text-sm font-bold">와인 사진</h2><div className="flex gap-2">
        {wineImages.length < MAX_WINE_IMAGES && (
          <button type="button" onClick={() => setIsPhotoSourceOpen(true)} className="flex size-[90px] flex-col items-center justify-center rounded-[10px] border border-dashed border-[#d9d9d9] bg-[#f2f2f2] text-black/20"><b className="text-[26px] font-normal leading-none">+</b><span className="text-[11px]">{wineImages.length}/3</span></button>
        )}
        {wineImages.map(({ id, url }, index) => (
          <div key={id} className="relative flex size-[90px] items-center justify-center overflow-hidden rounded-[10px] border border-[#d9d9d9] bg-[#f2f2f2]"><img src={url} alt="선택된 와인" className="h-[91px] w-[66px] object-cover" /><button type="button" aria-label="사진 삭제" onClick={() => removeWineImage(index)} className="absolute right-1 top-1 size-5 rounded-full bg-black/50 text-xs text-white">×</button></div>
        ))}
        {wineImages.length < MAX_WINE_IMAGES && (
          <button
            type="button"
            ...
            className={`flex size-[90px] flex-col items-center justify-center gap-[7px] rounded-[10px] text-[13px] leading-4 text-white transition-colors ${
              canGenerateImage || isGeneratingImage ? 'bg-[#831317]' : 'cursor-not-allowed bg-[#d9a3a5]'
            }`}
          >
