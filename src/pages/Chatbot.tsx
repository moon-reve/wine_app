...
지금 바로 한 가지를 추천한다면 새우 오일 파스타와의 조합을 가장 추천드려요. 처음 즐기는 분들도 부담 없이 만족도가 높은 페어링입니다.`,
  recommendation:
    "원하는 음식, 분위기, 가격대를 알려주시면 조건에 잘 맞는 와인을 추천해드릴게요.",
  wine: "어떤 와인이 궁금하신가요? 이름을 알려주시면 알려드릴게요.",
};
...
const followUpSuggestions: Record<ConversationKind, string[]> = {
  ...
  party: ["Cabernet으로 할래", "Chardonnay로 할래", "Cava로 할래"],
  white: [
    "새우 오일 파스타를 선택할게",
    "연어 카르파초로 선택할게",
    "조개 술찜으로 선택할게",
...
function getAnswer(value: string, kind: ConversationKind, mapPlace?: string) {
  if (mapPlace)
    return `네이버 지도에서 ${mapPlace} 열기 ↗\n\n카카오맵에서 열기 ↗`;
  const selectedFood = Object.keys(foodRecipes).find((food) =>
    value.includes(food),
  );
...
// 스크롤바를 숨긴 가로 스크롤 영역은 트랙패드 가로 스와이프가 없으면 스크롤할
// 수단이 없다. 마우스 드래그와 세로 휠 입력을 가로 스크롤로 매핑해 준다.
function useHorizontalScroll<T extends HTMLElement>() {
  ...
  const onWheel = (event: ReactWheelEvent<T>) => {
    const el = ref.current;
    if (!el) return;
  ...
  // 드래그가 있었으면 그 뒤의 click을 눌러서, 버튼이 즉시 선택되는 걸 막는다.
  ...
  return (
    <div className="space-y-7 py-1 text-[16px] leading-[1.55] tracking-[-0.32px]">
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="block w-fit border-b-2 border-dotted border-black pb-0.5"
      >
        네이버 지도에서 {place} 열기 ↗
      </a>
      <a
...
export default function Chatbot() {
  ...
  const initialWineQuestionHandledRef = useRef(false);
  ...
  useEffect(() => {
    const wine = locationState?.wineQuestion;
    if (!wine || initialWineQuestionHandledRef.current) return;

    initialWineQuestionHandledRef.current = true;
    setTurns([
      {
        id: 1,
        question: buildWineQuestion(wine),
        kind: "wine",
        answer: buildWineAnswer(wine),
      },
    ]);
    setResponseId(1);
  }, [locationState?.wineQuestion]);
  ...
  const submitQuestion = (value: string) => {
    ...
    if (!trimmed) return;
    const kind = matchConversation(trimmed);
    const mapPlace = getMapPlace(trimmed);
    setTurns((current) => [
      ...current,
      {
        id: responseId + 1,
        ...
        kind,
        mapPlace,
        answer: getAnswer(trimmed, kind, mapPlace),
      },
    ]);
  ...
  const chooseSuggestion = (suggestion: string) => {
    submitQuestion(suggestion.replace("\n", " "));
  };
    ...
    >
      ...
      <header className="absolute inset-x-0 top-0 z-30 h-[70px] bg-white/90 backdrop-blur-sm">
        <button
          ...
          aria-label="뒤로 가기"
          onClick={handleBack}
          className="absolute top-2 left-2 z-30 flex size-12 cursor-pointer touch-manipulation items-center justify-center rounded-full"
        >
          <img src={chatbotBack} alt="" aria-hidden="true" className="size-6" />
        </button>
        <h1 className="pointer-events-none absolute inset-x-0 top-[24px] text-center text-lg leading-none font-bold tracking-[-0.54px]">
          AI 챗봇
        </h1>
        <button
          type="button"
      ...
      >
        ...
        >
          ...
          <p className="absolute inset-x-0 top-[347px] text-center text-2xl leading-normal font-semibold tracking-[-0.48px]">
            안녕하세요. Sora Choi님
          </p>
          <p className="absolute inset-x-0 top-[387px] text-center text-[40px] leading-normal tracking-[-0.8px]">
            무엇이 궁금하신가요?
          </p>
          <div
            ref={initialSuggestionsScroll.ref}
            onMouseDown={initialSuggestionsScroll.onMouseDown}
            onMouseUp={initialSuggestionsScroll.onMouseUp}
            onMouseLeave={initialSuggestionsScroll.onMouseLeave}
        ...
        <div className="px-5">
          {turns.map((turn, index) => {
            ...
            const isComplete =
              !isLatest || shownAnswer.length === turn.answer.length;
            return (
              <section key={turn.id} className="pt-6">
                ...
                <article className="mt-8 min-h-6 pb-5 text-[16px] leading-[1.55] tracking-[-0.32px] whitespace-pre-wrap">
                  {turn.mapPlace && isComplete ? (
                    <MapLinks place={turn.mapPlace} />
                  ) : (
                    shownAnswer
                  )}
                  {!isComplete && (
                    <span
                      aria-hidden="true"
                      className="ml-0.5 inline-block h-[1.15em] w-[2px] animate-pulse bg-black align-[-0.18em]"
                    />
                  )}
                </article>
                {isComplete && (
                ...
                {isLatest && isComplete && !turn.mapPlace && (
                  <div
                    ref={followUpSuggestionsScroll.ref}
                    onMouseDown={followUpSuggestionsScroll.onMouseDown}
                    onMouseUp={followUpSuggestionsScroll.onMouseUp}
                    onMouseLeave={followUpSuggestionsScroll.onMouseLeave}
